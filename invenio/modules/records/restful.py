...
class BaseRecordResource(Resource):
    ...
    def __init__(self):
        ...
        self.mimetypes = dict()
        for f in output_formats.itervalues():
            if f.get('mime_type') is not None:
                self.mimetypes[f['mime_type']] = f['code']
                self.representations[f['mime_type']] = (
...
class RecordListResource(BaseRecordResource):
    ...
    def get(self):
        ...
        records_in_requested_format = []
        rec_ids = SearchEngine(query).search(user_info=current_user)
        if rec_ids:
            for rec_id in rec_ids:
                result = format_record(recID=rec_id, of=output_format)
                records_in_requested_format.append(result)
